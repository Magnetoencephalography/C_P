...
def main():
    for line in open(cfg.SET_PARAMETER, 'r'):
        ...
        if not os.path.exists(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet'):
            os.makedirs(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet')
        ...
        for e_i in range(len(SET_EVENTID)):          
            ...
            power,itc = mne.time_frequency.tfr_morlet(epochs, n_cycles=3, return_itc=True,freqs=frequencies, decim=3,n_jobs=n_jobs)

            power.apply_baseline((-0.4, 0),mode='zscore')
            itc.apply_baseline((-0.4, 0),mode='zscore')
            
            power.save(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet/'+SET_EVENTNAME[e_i]+'_power', overwrite=True)
            itc.save(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet/'+SET_EVENTNAME[e_i]+'_itc', overwrite=True)
            
            fig=power.plot_topo(baseline=(-0.4, 0), mode='zscore', title='Average power',fig_facecolor='w', font_color='k', border='k')
            fig.savefig(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet/power_plot_topo'+ SET_EVENTNAME[e_i] +'.png',dpi=300)
            plt.close()            

            fig=itc.plot_topo(baseline=(-0.4, 0), mode='zscore', title='Average itc',fig_facecolor='w', font_color='k', border='k')
            fig.savefig(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet/itc_plot_topo'+ SET_EVENTNAME[e_i] +'.png',dpi=300)
            plt.close()    
            ...
            plt.title('temp_itc')
            plt.colorbar()
            plt.savefig(cfg.SET_RESULT_DIR + '/' + SUBJECT+'/wavelet/power_itc_'+ SET_EVENTNAME[e_i] +'.png',dpi=300)
            plt.close()    
